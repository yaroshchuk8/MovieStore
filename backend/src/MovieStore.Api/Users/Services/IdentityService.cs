    ...
    : IIdentityService
{
        ...
        Role role)
    {
        ...
        await context.UserProfile.AddAsync(domainUser);
    ...
    public async Task<ErrorOr<AuthTokens>> RefreshAuthTokensAsync(string accessToken, Guid refreshToken)
    {
        ...
        var storedToken = await context.RefreshToken
            .AsNoTracking()
            .FirstOrDefaultAsync(predicate: rt => rt.Value == refreshToken);
        ...
        var domainUser = await context.UserProfile.FirstOrDefaultAsync(u => u.IdentityUserId == identityUser.Id);
        ...
        context.RefreshToken.Update(storedToken);
